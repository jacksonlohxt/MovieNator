...
- `src/producer-consolidation.js`, `test/producer.test.js`, and `/v1/producer-source-bundles` define the strict bounded Producer Intake slice; one `primary_screenplay` and up to twelve PDF/TXT sources produce stable source hashes, allowlisted classifications, conflicts, citations, and read-only Markdown/JSON handoff exports. The older `/v1/producer-packets` label path remains compatibility-only.
