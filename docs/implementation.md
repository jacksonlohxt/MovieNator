...
- `src/parallel-search.js` is the credential-gated, config-off-by-default Parallel Search seam: it imports the real `parallel-web` SDK, builds up to a few bundle-relevant research topics from an already-built strict packet, and merges cited, clearly-labelled `externally_researched_fact` evidence rows (never a legal conclusion, approval, or booking) back into the packet. `createApp` composes it into the default producer packet builder only when `PARALLEL_API_KEY` is configured; without it, the packet builds exactly as before with `external_evidence: []`. See `docs/parallel-search-enrichment.md` for the full contract, configuration, and test seam.
