...
- `src/server.js` provides the API and same-origin SSE. Work is queued after `202 Accepted`; the browser reads safe projections, evidence records, document metadata, and bounded citation excerpts only. The original `/v1/runs` and evidence routes remain the Audience Data Readiness workflow. Producer Intake uses the strict manifest route `/v1/producer-source-bundles` plus `/v1/producer-source-bundles/{bundle_id}/packets`, while the older `/v1/producer-packets` multipart route remains compatible for legacy source labels. Packet handoff export is an allowlisted Markdown or JSON read-only view.
