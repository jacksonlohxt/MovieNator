...
- `src/logic-contracts.js`, `src/tool-registry.js`, `src/mcp-database.js`, `src/orchestrator.js`, `src/workflow-state.js`, and `src/agent-runtime-boundary.js` define the Phase 4 resumable-state contracts, allowlisted local read tools, bounded proposal execution, and future runtime adapter boundary. `src/producer-agent-boundary.js` adds the product-owned read-only Producer Intake boundary for the Google adapter. See `docs/phase4-state-logic-hosting.md` and `docs/google-agent-runtime.md` for the detailed contracts.
- `src/server.js` provides the API and same-origin SSE. Work is queued after `202 Accepted`; the browser reads safe projections, evidence records, document metadata, and bounded citation excerpts only. The original `/v1/runs` and evidence routes remain the Audience Data Readiness workflow. Producer Intake uses the strict manifest route `/v1/producer-source-bundles` plus `/v1/producer-source-bundles/{bundle_id}/packets`, which emits the PRD-target `producer-intake-decision-packet@1` contract (source/version manifest, exact facts, conflicts, evidence states, and the full `contract_version`/`bundle_manifest_hash`/`source_manifest_hash` provenance object), while the older `/v1/producer-packets` free-label multipart route remains a stable, unrenamed `producer-decision-packet@1` compatibility alias for legacy source labels. Packet handoff export is an allowlisted Markdown, JSON, or CSV read-only view.
- `src/parallel-search.js` is the credential-gated, config-off-by-default Parallel Search seam: it imports the real `parallel-web` SDK, builds up to a few bundle-relevant research topics from an already-built strict packet, and merges cited, clearly-labelled `externally_researched_fact` evidence rows (region-scoped location suggestions, permit guidance, labor/stunt guidance, and vendor/day-rate ranges, never a legal conclusion, approval, booking, budget total, or tier verdict) back into the packet. The strict deterministic packet also emits rich scene rows, controlled production-element details, explicit coverage gaps, and budget-risk observations routed to humans. `target_region` defaults to Singapore but remains configurable per strict packet request. `createApp` composes Parallel only when `PARALLEL_API_KEY` is configured; without it, the packet builds exactly as before with `external_evidence: []`. See `docs/parallel-search-enrichment.md` for the full contract, configuration, and test seam.
- `web/` is a dependency-free responsive browser route whose primary flow is Upload script, Tell us what you want, Create brief, then Read or copy the result. Audience Ask, Clarify, Role Run, Decision, Evidence, Recovery, and the bounded role map are secondary Developer details. The producer consolidation surface accepts one required `primary_screenplay` and up to twelve bounded PDF/TXT sources, visibly labels deterministic local/no-credentials mode, and returns a versioned packet with source hashes, evidence classifications, exact facts, conflicts, open questions, and a producer handoff projection.
