...
- The brief worker receives the bounded condensed excerpts, their citation IDs and locations, and the server-owned Script Brief prompt. It validates `grounded-script-brief@2` plus the versioned `producer-intelligence@1` section, preserves exact screenplay location wording, requires citations for material claims, and rejects unknown citations, unsafe text, unbounded output, and side-effect language.
- The browser primary flow is Upload script, Tell us what you want, Create brief, then Read or copy the result. The result contains a logline, approximately 100-word synopsis, main characters, setting/tone/themes, production details, bounded source-grounded producer intelligence, open questions, and expandable citations. Producer intelligence covers scene headings and exact location wording, cast demands, established production signals, risks, and explicit gaps without inferring venues, permits, budgets, schedules, or legal conclusions. There is intentionally no human approval screen because this MVP only reads the uploaded source and projects a bounded brief.
